...
export default function GeneratingOverlay() {
    ...
    for (const shape of allShapes) {
      ...
      if (meta?.source === 'generating') {
        ...
        if (bounds) {
          ...
          const screenBottomRight = editor.pageToScreen({ x: bounds.x + bounds.w, y: bounds.y + bounds.h })
  ...
  useEffect(() => {
    ...
    const disposables = [
      editor.store.listen(() => updateGeneratingShapes()),
    ]
  ...
  return (
    <>
      {generatingShapes.map((shape) => (
        <div
          key={shape.id}
          className="fixed pointer-events-none z-40 rounded-lg overflow-hidden"
          style={{
            left: shape.screenX,
            top: shape.screenY,
